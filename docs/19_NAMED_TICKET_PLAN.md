...
## Phase 4 — จุดสแกนด้วยกล้อง + role STAFF + QR ทนเน็ตหาย (rev 42, 2026-08-27)

- **เจ้าหน้าที่ = role `STAFF`** — แอดมินแต่งตั้งด้วยอีเมลที่ `/admin/staff` (บัญชีต้องสมัครไว้ก่อน) · เข้าได้เฉพาะ `/staff/checkin` ไม่เห็นหน้าแอดมินอื่น · ADMIN สแกนได้เหมือนกัน · ถอนสิทธิ์มีผลทันที (guard เช็ค DB) · บันทึก `Ticket.checkedInById` ว่าใครสแกน
- **กล้อง** — `qr-scanner` ในเบราว์เซอร์ (มือถือกล้องหลัง, ต้อง https) · ช่องพิมพ์/ปืนสแกนยังอยู่เป็นทางสำรอง · อ่านได้แล้วเรียก `checkInTicket` ตัวเดิม
- **จุดสแกนผูกกับงาน** (audit rev 42 — High): จนท. ต้องเลือกงานก่อน (`?concert=<id>`) · server ปฏิเสธตั๋วของงานอื่น และตั๋วนอกกรอบเวลา `[eventAt − 12 ชม., eventAt + 6 ชม.]` (env `CHECKIN_OPEN_BEFORE_HOURS`/`CHECKIN_CLOSE_AFTER_HOURS`) **ก่อน** claim — สแกนผิดประตูไม่เผาตั๋ว · ผลขึ้นกล่องเหลือง "ตั๋วคนละงาน" บอกชื่อทั้งสองงาน · เหตุผล: HMAC ของ QR พิสูจน์แค่ "ตั๋วใบนี้สดจริง" ไม่ได้พิสูจน์ "ตั๋วของประตูนี้" — ตั๋ว ฿1 ของคอนทดสอบเคยผ่านประตูคอนราคาเต็มได้ในทางทฤษฎี
- **QR ตั๋วทนเน็ตหาย** — client ขอภาพล่วงหน้า 10 ช่วง (5 นาที) แล้วหมุนเองตามเวลา · กติกาตรวจ ±1 ช่วงเดิม → แคปหน้าจอ 1 ภาพยังตายใน ≤60 วิ แต่เน็ตหาย/จอล็อกไม่ทำให้เข้างานไม่ได้ภายใน 5 นาที · secret ยังไม่ออกจาก server
- **ไม่ทำ (ตั้งใจ):** จุดสแกนตรวจออฟไลน์ (ต้องเอา secret ไปไว้ที่เครื่องสแกน) · โอน/ขายต่อ — กฎเหล็กเดิมยังอยู่
