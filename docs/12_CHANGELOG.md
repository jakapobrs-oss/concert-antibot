...
## [Revision 42 — จุดสแกนเช็คอินด้วยกล้อง + role STAFF แยกเจ้าหน้าที่ออกจากแอดมิน + QR ตั๋วทนเน็ตหาย] — 2026-08-27

**ที่มา:** user-test ตามใบงาน (Artifact "User-Test Concert Anti-Bot") ชี้ว่า "ลืมตัวอ่าน QR สำหรับสแกนตั๋ว" — หน้าเช็คอินเดิมรับได้แค่ปืนสแกน/วางข้อความ และต้องล็อกอินเป็น ADMIN (เจ้าหน้าที่หน้างานจะเห็นทุกหน้าแอดมิน) · คำถามต่อเนื่อง "QR หมุน 30 วิ คลาดเคลื่อนไหม" → ไม่ (server ทั้งสองฝั่ง + รับ ±1 ช่วง) แต่ **เน็ตหาย/จอล็อกทำให้ QR ตายใน ~1 นาที** = ความเสี่ยงหน้างานจริง

**ทำ**
- **role `STAFF`** (`prisma/schema.prisma` + migration `20260827130000_add_staff_role_ticket_checked_in_by`): เจ้าหน้าที่หน้างานทำได้อย่างเดียวคือสแกน · `Ticket.checkedInById` บันทึกว่าใครสแกนใบไหน · `lib/admin-guard.ts` เพิ่ม `isVerifiedStaff/assertVerifiedStaff` (STAFF|ADMIN — เช็ค role กับ DB จริงเหมือนแอดมิน) · `lib/staff-role.ts` (pure) กติกา USER↔STAFF เท่านั้น **บัญชี ADMIN แตะจากหน้านี้ไม่ได้** (กันแอดมินสร้าง/ลดสิทธิ์กันเองจากหน้าเว็บ) · `app/actions/admin-staff.ts` (updateMany มีเงื่อนไข role เดิม กันสองแอดมินกดชน) + หน้า `/admin/staff` (แต่งตั้งด้วยอีเมล / ถอน 2 ขั้นไม่ใช้ window.confirm / นับใบที่สแกน) + ปุ่มบนแดชบอร์ด
- **`/staff/checkin`** (route group ใหม่ `(staff)` + layout guard เช็ค DB) — `/admin/checkin` เหลือเป็น redirect · **`lib/route-access.ts`** (pure, unit test) แทนกติกาที่เคยฝังใน `middleware.ts`: `/admin`=ADMIN · **`/staff`=แค่ล็อกอิน** (role ตัดสินที่ layout กับ DB — คนเพิ่งถูกแต่งตั้งเปิดลิงก์ได้ทันทีโดยไม่ต้องล็อกอินใหม่ ถอนแล้วเด้งทันที) · เมนูผู้ใช้เพิ่ม "จุดเช็คอิน" (อ่าน role จาก JWT → ขึ้นหลังล็อกอินใหม่ — ข้อความตอนแต่งตั้งบอกไว้)
- **กล้องสแกน** `components/checkin-camera.tsx` — dependency ใหม่ **`qr-scanner@1.4.2`** (nimiq: ถอดรหัสใน worker + ใช้ BarcodeDetector ถ้ามี, ~16 KB, โหลดตอนกด "เปิดกล้อง" เท่านั้น) · กล้องหลัง · กันสแกนซ้ำ 3 วิ + ไม่รับระหว่างกำลังตรวจ · ข้อความ error ตามสาเหตุ (ไม่อนุญาต/ไม่มีกล้อง/ถูกแอปอื่นใช้/ต้อง https) · `checkin-client.tsx` รวมกล้อง+ช่องพิมพ์ลง `submit()` เดียว + สั่นบอกผล (ผ่าน 1 ครั้ง / ไม่ผ่าน 5 จังหวะ)
- **QR ตั๋วทนเน็ตหาย** `lib/entry-code.ts` `entryCodeBatch()` (เพดาน `ENTRY_PREFETCH_WINDOWS`=10 ช่วง = 5 นาที) + action `getEntryCodes` (ภาพเป็นชุด) + `ticket-entry-qr.tsx` เขียนใหม่: ตำแหน่งภาพคิดจาก "เวลา" ไม่ใช่นับ timer (จอล็อก/สลับแอปกลับมาได้ภาพถูก) · refetch เมื่อเหลือ ≤3 ภาพ / `visibilitychange` / `focus` / `online` · ขอไม่ได้ → โชว์ภาพสุดท้าย + ป้าย "ออฟไลน์ — QR อาจหมดอายุ" · **กติกาตรวจไม่เปลี่ยน** (±1 ช่วง) — แค่ client รู้ล่วงหน้า 5 นาที (แคปหน้าจอ 1 ภาพยังตายใน ≤60 วิ เท่าเดิม, secret ยังไม่ออกจาก server)
- บทเรียนโค้ด: `fetchBatch().then(tick)` รันก่อน React commit state → ต้องอัปเดต ref เองทันที ไม่งั้น tick เห็น batch เก่าแล้วยิง server ซ้ำ (จับได้จากเทสเบราว์เซอร์ 5d)

**Audit (ultracode `money-audit` 2 lens สิทธิ์+race, 21 candidate → verify 10 ข้อ ก่อนชน spend limit) — แก้ในรอบเดียวกัน:**
- 🔴 **High — จุดสแกนไม่ผูกกับคอนเสิร์ต** (มีมาตั้งแต่ Phase 2 แต่ rev 42 ให้ STAFF+กล้อง auto-submit ยิ่งพลาดง่าย): ตั๋ว ฿1 ของคอนทดสอบ/ตั๋วงานที่จบไปแล้ว สแกนที่ประตูคอนราคาเต็ม → ขึ้นเขียว "ให้เข้างานได้" เหลือแค่สายตา จนท. อ่านชื่อคอนบรรทัดเล็ก → **แก้**: `lib/checkin-policy.ts` (pure) `decideCheckInScope` — ตั๋วต้องเป็นของงานที่จุดสแกนเลือก + อยู่ในกรอบ `[eventAt − CHECKIN_OPEN_BEFORE_HOURS(12), eventAt + CHECKIN_CLOSE_AFTER_HOURS(6)]` (env ใหม่ 2 ตัว มี default ไม่ต้องตั้งบน Vercel) · `checkInTicket({ qrText, concertId })` ตัดสิน **ก่อน** claim (สแกนผิดประตูไม่เผาตั๋ว) + `where` ของ updateMany ผูก concertId ซ้ำ · หน้า `/staff/checkin` ต้องเลือกงานก่อน (`components/checkin-concert-picker.tsx` เก็บใน `?concert=<id>` — รีเฟรช/ส่งลิงก์ให้ จนท. คนอื่นได้) ลิสต์เฉพาะงานที่ยังไม่เลยกรอบปิดสแกน · UI ผลปฏิเสธแบบ "คนละงาน/นอกเวลา" เป็นกล่องเหลืองบอกชื่อทั้งสองงาน + สั่นยาว 3 ครั้ง (ต่างจากแดง "ปฏิเสธจริง") · ชื่อคอนในกล่องเขียวยกเป็นตัวหนา · `CheckInResult` ฝั่งไม่ผ่านมี `kind` ให้ UI แยก
- 🟠 Medium — `ticket-entry-qr.tsx` promise chain (fetch→tick) รอดข้าม unmount ตั้ง timer ต่อไม่รู้จบ → ใส่ `alive` ref (ของเดิมมีแต่ถูกถอดตอนเขียนใหม่) · 🟠 Medium — กล้อง dedupe 3 วิ key ด้วยข้อความที่หมุนทุก 30 วิ → ใบที่เพิ่งผ่านถูกยิงซ้ำแล้วพลิกแดง "เช็คอินไปแล้ว" ตรงหน้าคน → key ด้วย **เลขบัตร** กัน 20 วิ + พัก 1.5 วิ หลังอ่านได้ทุกใบ
- 🟡 Low — rate-limit `getEntryCodes` 30/นาที/ผู้ใช้ + `checkInTicket` 300/นาที/เจ้าหน้าที่ (Redis `checkRateLimit` เดิม) · นาฬิกาเครื่องถอยหลัง → refetch แทนโชว์ภาพเก่า · แต่งตั้ง STAFF ด้วยอีเมลเทียบแบบ insensitive (ตอนสมัครไม่ได้ normalize — หน้าสิทธิ์สมาชิกยังใช้ lowercase+findUnique เหมือนเดิม ค้าง) · ยอมรับไว้: ชุด QR ล่วงหน้าขยายหน้าต่างแชร์ "ชุดภาพ" เป็น ≤5.5 นาที (เอกสารไว้แล้ว) · FK `checkedInById` ON DELETE SET NULL — ลบบัญชี จนท. ผ่าน `scripts/user-admin.ts` จะเสีย audit ว่าใครสแกน (บันทึกไว้ ไม่แก้)
- ยังไม่ dry (audit 1 รอบ, verify 12 ตัวหลังชน limit) — รอบถัดไปถ้าจะให้ครบ: `Workflow money-audit rounds:2`

**Hotfix หลัง deploy (session เทส Opus เจอ — `user-test-runs/2026-08-27-rev42/LOG.md`):** 🔴 High — หน้าตั๋ว**ออฟไลน์นานเกินชุด QR** (เริ่มตั้งแต่เหลือภาพ ≤3 ช่วง ≈ นาที 3.5) แล้ว **วนยิง `getEntryCodes` ไม่หยุด** — วัดจริง 59,153 คำขอใน 3 นาทีจากแท็บเดียว, เน็ตกลับแล้วหน้าไม่ฟื้น (โควต้า 30/นาทีถูกกินหมด) · กลไก: `tick()` ล้าง timer ทุกครั้ง แล้วเรียก `fetchBatch().then(tick)` ทันที → fetch ล้ม → `.then(tick)` → วนไม่มีดีเลย์ (ธง `fetching` กันไม่อยู่เพราะ `finally` ปลดก่อน) · **แก้**: fetch ที่ล้ม "ไม่" ปลุก tick · ยิงซ้ำได้เมื่อถึง `nextRetryAt` เท่านั้น (ถอยหลังทวีคูณ 10→20→40→…→120 วิ) · timer ตัวเดียวตื่นที่ min(ขอบช่วงถัดไป, เวลา retry) · `online` รีเซ็ต backoff แล้วขอทันที · regression: `scripts/test-rev42-gaps.ts` (`pnpm test:rev42-gaps`, 25 เช็ค G1–G8 — เขียนโดย session เทส เก็บเข้า repo; G3b วัด "ใบละ ≤1 ครั้ง" เพราะหน้าตั๋วมี QR หลายใบแต่ละตัวมีชุดของตัวเอง + G3b2 ไม่ยิงเพิ่มระหว่าง backoff)
- 🔴 **Hotfix รอบ 3 — Critical: กล้องสแกนของ rev 42 "ใช้ไม่ได้ 100% บน prod ตั้งแต่ deploy" (user ลอง iPhone จริง 27 ส.ค. ดึก: "กล้องเปิดแต่ไม่สแกน"):** เทสกล้องปลอม `scripts/test-camera-scan.ts` (`pnpm test:camera-scan` — Chromium `--use-file-for-fake-video-capture` ไฟล์ .y4m ที่วาด QR ของตั๋วลงไป; โหมด `NO_BARCODE_DETECTOR=1` บังคับสาย worker ที่ Safari ใช้) จับได้ 2 ตัวใน `next.config.ts` ที่ทำให้กล้องไม่มีทางทำงานในทุกเบราว์เซอร์ทุกเครื่อง: (1) `Permissions-Policy: camera=()` (header ทั้งเว็บ) ปิดกล้องทุกหน้า — Chrome/Android ขอกล้องไม่ผ่านตั้งแต่แรก → `camera=(self)` (2) CSP ไม่มี `worker-src` → qr-scanner สร้าง Web Worker ถอดรหัสจาก `blob:` ถูกบล็อกเงียบ ๆ — Safari หย่อนเรื่อง Permissions-Policy จึงเปิดกล้องได้แต่ไม่มีตัวถอดรหัสทำงาน → เพิ่ม `worker-src 'self' blob:` · **deploy `pses064cp`/`qdgxzfiwe`/`v4k7h9lvc` ก่อนหน้านี้กล้องใช้ไม่ได้เลย** — ช่องพิมพ์/ปืนสแกนยังใช้ได้ · regression `tests/unit/security-headers.test.ts` (อ่าน header จาก next.config ตรง ๆ จับได้ใน ms) · ปรับ `checkin-camera.tsx` ตามรีวิว session เทส: เลิก display:none ตอน start (iOS ไม่เล่น video ใต้ display:none) + รอ 2 RAF · preload ไลบรารีตอน mount ลด await ก่อน getUserMedia (user gesture บน iOS) · object-contain แทน object-cover (ภาพที่เห็น = พื้นที่สแกน) · สแกนทั้งเฟรม downscale ≤960 แทนสี่เหลี่ยมกลาง 2/3→400px · คำแนะนำเล็งเมื่อไม่เจอ QR 3 วิ · หน้าตั๋ว: QR ใหญ่ขึ้น (128px) + แตะขยายเต็มจอ (`image-rendering: pixelated`) + บอกเร่งความสว่าง · หลักฐาน: `pnpm test:camera-scan` 6/6 ทั้ง 2 โหมด · unit **64 ไฟล์ 756/756** (ใหม่ `security-headers` 3) · tsc 0 · lint 0
- **กฎเทสที่ได้จากวันนี้ (เขียนไว้ใน CLAUDE.md ด้วย):** เช็ค "ล้มแล้วขึ้นข้อความสวย" ต้องมีคู่กับเช็ค "ทางสำเร็จเกิดขึ้นจริง" เสมอ — `test:staff-checkin` 3c (headless ไม่มีกล้อง → ข้อความชัด) ผ่านตลอดทั้งที่ฟีเจอร์พังสนิท เพราะมันจริงทั้งตอนระบบดีและพัง (แบบเดียวกับ G3a `body.innerText` ผลบวกลวง) (1) `getEntryCodes` มี timeout 8 วิ (Promise.race) ให้ `finally` รันเสมอ + **generation counter** — ผลของคำขอที่ถูกทิ้ง/มาช้าถูกเมินทั้ง then/catch/finally (ป้องกันคำขอค้างจากตอนออฟไลน์มาล้มทีหลังแล้วเขียนทับสถานะ) · handler `online` ทิ้งคำขอค้างก่อนขอใหม่ (2) **การเติมชุดใหม่เคยยิงซ้ำ 1 คำขอทุกครั้ง** — tick เทียบ index ของชุดใหม่ (0) กับ `lastIdx` ของชุดเก่าแล้วเข้าใจว่านาฬิกาถอยหลัง → รีเซ็ต `lastIdx` ตอนติดตั้งชุด (probe: n ขึ้นทีละ 1 แล้ว) (3) log `[ticket-qr]` (console.debug ฝั่ง client) + `[entry-codes] ok|reject` (ฝั่ง server) ไว้ไล่เคสออฟไลน์โดยไม่ต้องแก้โค้ด · ผล prod-mode: ออฟไลน์ 2 คำขอ/4 นาที · เน็ตกลับ → ชุดใหม่มาถึงทันที QR ตรงกับที่ประตูคาด byte ต่อ byte ยาวถึงนาที 8.5 · **"ป้ายออฟไลน์ค้าง" ที่รายงานตอนแรกเป็นผลบวกลวงของ `body.innerText` ในตัววัด ไม่ใช่บั๊ก** (ป้ายจริงใน `<p>` = "QR หมุนอัตโนมัติ" ตั้งแต่จุดวัดแรก) → G3a/G3c ใน `test-rev42-gaps.ts` เปลี่ยนไปอ่านป้ายจาก `<p>` ตรง ๆ · อาการ "หน้า reload เอง" ที่เห็นครั้งเดียวบน dev = HMR ของ `next dev` ไม่เกิดบน prod-mode · probe รอเวลาจริงของ session เทสเก็บเข้า repo เป็น `scripts/probe-offline-loop.ts` (`pnpm probe:offline-loop`, รันมือ ~6–7 นาที ไม่เข้า CI) — ยืนยันเพิ่มว่า QR ที่หมุนเองตอนออฟไลน์ตรงกับที่ประตูคาด byte ต่อ byte และสแกนผ่านจริง · ตั๋วเร็วกว่ากรอบ/คืนแล้ว/QR เก่า 2 นาที ถูกปฏิเสธโดยไม่เผาตั๋ว · งานที่จบเกิน 6 ชม. ไม่โผล่ในตัวเลือกและยิง `?concert=` ตรงก็ไม่เปิด · เมนู "จุดเช็คอิน" โผล่หลังล็อกอินใหม่

**หลักฐาน:** unit **63 ไฟล์ 753/753** (ใหม่ `route-access` 12 · `staff-role` 6 · `staff-guard` 8 · `admin-staff-action` 11 · `entry-code-batch` 6 · `checkin-policy` 7) · tsc 0 · lint 0 error · **`pnpm test:staff-checkin` (เบราว์เซอร์จริง 27 เช็ค)**: USER ถูกเด้งทั้ง /staff /admin · แอดมินแต่งตั้ง/ถอนผ่าน UI (แต่งตั้ง ADMIN ถูกปฏิเสธ) · STAFF ใช้จุดสแกนได้ทันทีโดยไม่ล็อกอินใหม่ และยังเข้า /admin ไม่ได้ · ยังไม่เลือกงาน = ไม่มีช่องสแกน · แผงกล้อง dynamic import โหลดได้ (headless ไม่มีกล้อง → ข้อความชัด) · **ตั๋วงาน A ที่ประตูงาน B → กล่องเหลืองบอกชื่อทั้งสองงาน + ตั๋วไม่ถูกเผา** · วาง QR จริงที่ประตูถูก → ชื่อผู้ถือ + `checkedInById` · ซ้ำ/มั่วถูกปฏิเสธ · หน้าตั๋วขอชุดเดียวแล้วหมุนเองข้าม 30 วิ ไม่ยิง server ซ้ำ · **กล้องจริงบนมือถือยังไม่ได้ลอง** — user ต้องลองบน prod หลัง deploy (ต้อง https) · `next build` local ไม่ได้รัน (dev server ค้างพอร์ต 3000 ใช้ `.next` เดียวกัน) — พึ่ง CI build ตอน push

**ยังไม่ทำ (ตั้งใจ):** จุดสแกนตรวจแบบออฟไลน์ (ต้องเอา secret ไปไว้ที่เครื่องสแกน = ขัดกติกา secret อยู่ server) · ตั้ง/ลด ADMIN จากหน้าเว็บ · เสียงติ๊กตอนสแกน

---

## [Revision 41 — หน้าแก้ไขคอนเสิร์ตของแอดมิน (ชื่อ/วัน/ช่วงขาย/สถานะ/slug/โปสเตอร์) + ลบคอนที่ยังไม่มีคำสั่งซื้อ] — 2026-08-27

**ที่มา:** ประเมินความสมบูรณ์หลัง rev 40 — แอดมินแก้ข้อมูลคอนเสิร์ตไม่ได้เลย (มีแค่เปิด-ปิดขาย + ผัง + รอบ) พิมพ์ผิดต้องเข้า DB · user สั่งทำข้อนี้ก่อนเพราะกรรมการมักลองกดตอนสอบ

**ทำ**
- `lib/concert-form.ts` (pure, ใหม่): `parseConcertForm()` กติกาเดียวใช้ทั้งสร้าง+แก้ — ฟิลด์บังคับ · datetime-local เป็นเวลาไทย · ปิดขาย > เริ่มขาย · **ปิดขาย ≤ เวลาแสดง** (กติกาใหม่ กันเคส "คอนพี่เจี๊ยบ" ขึ้นกำลังขายหลังงานจบ) · จำกัดตั๋ว 1–20 · โปสเตอร์ต้อง http(s) · `slug` a-z0-9- (ว่าง = คงเดิม) · `status` ในลิสต์ · `canDeleteConcert()` (ลบได้เมื่อไม่มีคำสั่งซื้อ — Order ไม่ cascade) · `changesAffectingBuyers()` (เปลี่ยนวัน/สถานที่ทั้งที่มีคนจ่ายแล้ว → เตือนให้แจ้งเอง) · `lib/local-datetime.ts` + `toThaiDateTimeLocal()` (คู่กลับ ไว้เติมค่าเริ่มต้นฟอร์ม)
- `app/actions/concert.ts`: `updateConcertAction(concertId, prev, formData)` / `deleteConcertAction` (ใช้กับ `useActionState` — error/คำเตือนโชว์บนหน้าเดิม) · เช็ค slug ซ้ำก่อนเขียน · ล้างแคช ISR ครบ (รายการ/หน้ารายละเอียด slug เก่า+ใหม่/หน้าแรก/แอดมิน) — `updateConcertStatus` ก็ล้างหน้าสาธารณะแล้ว (เดิมล้างแค่แอดมิน ผู้ชมเห็นช้าได้ถึง 60 วิ) · `createConcert` ใช้ parser เดียวกัน (+ รับ `coverImageUrl`)
- UI: `app/(admin)/admin/concerts/[id]/edit/page.tsx` + `components/concert-edit-form.tsx` (client) — ฟอร์มเต็ม + select สถานะ (บอกด้วยว่าหน้าเว็บเทียบความจริงอีกชั้น) + โซนลบ 2 ขั้น (ไม่ใช้ window.confirm) · ปุ่ม "แก้ไขข้อมูล" บนหน้ารายละเอียด + "แก้ไข" ทุกแถวในรายการ + แถบ "ลบคอนเสิร์ตแล้ว" หลัง redirect
- **ยังไม่ทำ (ตั้งใจ):** แก้โซน/ราคา (อยู่ที่หน้าผัง) · อัปโหลดรูปโปสเตอร์ (รับเป็นลิงก์) · ลบคอนที่มีคำสั่งซื้อ (ให้ตั้ง "จบงาน"/"ฉบับร่าง" ซ่อนแทน)

**หลักฐาน:** unit **57 ไฟล์ 705/705** (ใหม่ `concert-form` 12) · tsc 0 · lint 0 error · `next build` local ผ่าน · ยังไม่ได้กดจริงบน prod (ต้องล็อกอินแอดมิน — user ทดสอบหลัง deploy)
