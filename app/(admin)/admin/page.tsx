...
import { Music2, Flame, Users, Ticket, ShieldCheck, BarChart3, ScanLine, Undo2, Gauge, BadgeCheck, UserCog } from "lucide-react";
...
export default async function AdminDashboard() {
  ...
  const queueLines =
    queues.length > 0
      ? queues.map((q) => `  - ${q.title}: รอ ${q.waiting} คน, เข้าแล้ว ${q.admitted} คน`).join("\n")
      : "  ไม่มีคอนเสิร์ตที่กำลังขายอยู่";

  const adminPageContext = [
    `=== สถิติแดชบอร์ด ณ ปัจจุบัน ===`,
    `คอนเสิร์ตทั้งหมด: ${stats.concertCount} (กำลังขาย: ${stats.onSaleCount})`,
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link href="/staff/checkin">
            <Button variant="outline" leftIcon={<ScanLine className="size-4" />}>
              เช็คอินหน้างาน
            </Button>
          </Link>
          <Link href="/admin/staff">
            <Button variant="outline" leftIcon={<UserCog className="size-4" />}>
              เจ้าหน้าที่หน้างาน
            </Button>
          </Link>
          <Link href="/admin/refunds">
            <Button variant="outline" leftIcon={<Undo2 className="size-4" />}>
              งานคืนเงิน
            </Button>
          </Link>
        ...
        {/* Gemini AI Assistant */}
        <div className="mt-6">
          <AdminChatPanel pageContext={adminPageContext} />
        </div>
      </main>
    </>
  );
