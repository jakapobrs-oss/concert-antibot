...
- **Unit**: `tests/unit/*.test.ts` — 48 ไฟล์, 614 cases (รันจริง 2026-08-27 หลังรวม rev 31–34 — เพิ่ม `queue-soldout-gate`, `turnstile`, `money`, `local-datetime`, `email-signup-gate`, `seed-policy`, `credentials-auth-verify-flag`, `canonical-host`, `consent`; rev 28 +4, rev 29 +6, rev 30 +4, rev 31 +11 seed-policy, rev 32 +6 email-signup-gate/+4 credentials-auth-verify-flag, rev 33 +7 canonical-host, rev 34 +6 consent), Vitest, mock ล้วนไม่ต้องมี DB/Redis จริง (`pnpm test:run` — **ห้าม `pnpm test` = watch ค้าง**)
