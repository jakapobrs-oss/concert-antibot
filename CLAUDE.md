...
| **5. Admin & Cron** | `lib/{admin-guard,admin-stats}.ts` · `app/(admin)/*` · `app/actions/concert.ts` (สร้างคอนเสิร์ต — slug ผ่าน `lib/slug.ts`: ไทยล้วน → `concert-<id>`, ซ้ำ → `<slug>-<id>`; rev 39 แก้บั๊ก slug ว่างกดเข้าไม่ได้) · `app/api/cron/sweep/` (Vercel Cron รายวัน — `vercel.json` `0 0 * * *` ลิมิต Hobby) · `scripts/sweep-orders.ts` |
