...
| **(cross-cutting) สถานะที่แสดงของคอนเสิร์ต** | `lib/concert-display.ts` (pure) — `deriveDisplayStatus()` = status ใน DB เทียบความจริง (ไม่มีโซน → NOT_READY "ยังไม่พร้อมขาย" · เลย `saleEndAt` → ENDED · ก่อน `saleStartAt` → SCHEDULED) + `minZonePrice()` (null เมื่อไม่มีโซน — เดิม `Math.min([])` = Infinity โชว์ ฿∞) · ใช้ใน `components/concert-card.tsx`, `app/(public)/concerts/[slug]/page.tsx` (ป้าย+CTA+แถบ `?cancelled=1`), `app/(public)/concerts/[slug]/queue/page.tsx` (เปิดห้องรอเฉพาะ ON_SALE จริง) — rev 37 (user-test #40) · **อย่าเช็ค `concert.status === "ON_SALE"` ตรง ๆ ในหน้า public อีก** ใช้ helper นี้ |
