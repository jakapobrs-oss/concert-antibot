...
| DB/ORM | **PostgreSQL 16** ผ่าน **Prisma 6.1.0** — schema เดียวที่ `prisma/schema.prisma` (701 บรรทัด, **21 models** · 14 enums — เช็ค 2026-08-25 หลัง merge สาย presale; migration 16 ตัว — ตัวที่ 16 เป็น data-fix slug ว่าง rev 39) |
| Cache/Queue/Lock | **Redis** ผ่าน `ioredis` — hand-rolled ทั้งหมด (ไม่ใช่ BullMQ), ใช้ทำ queue/seat-lock/rate-limit/load-shed |
| Payment | `promptpay-qr`+`qrcode` (สร้าง QR) + ตรวจสลิปผ่าน **สวิตช์ `SLIP_PROVIDER`** (rev 40: `lib/slip-verify.ts` จุดเข้าเดียว → adapter `lib/easyslip.ts` \| `lib/slipok.ts` (SlipOK ฟรี 100 สลิป/เดือน — header `x-authorization`, `POST /api/line/apikey/{branchId}` multipart `files`) · กติกาเงินร่วมใน `lib/slip-policy.ts`; ตั้งคีย์เฉพาะเจ้าที่เปิดใช้) — EasySlip REST client มือเขียน (`lib/easyslip.ts` — rev 39: ส่งรูป multipart `file`, map รหัส error → ข้อความผู้ใช้/แอดมิน + log `[PAYMENT][EASYSLIP]`, `fetchEasySlipAccountStatus()` /me → การ์ดแดชบอร์ดแอดมิน + boot-warn ใน `instrumentation.ts`; **แอปฟรี EasySlip มีวันหมดอายุ + โควต้า 50 — หมดอายุ = จ่ายเงินไม่ได้ทั้งเว็บ (เคยเกิด 10 มิ.ย.–27 ส.ค. 2026)**) — **ไม่มี Stripe/Omise** แม้เอกสารเก่าบางไฟล์จะพูดถึง |
...
| Subsystem | อยู่ที่ |
|---|---|
| **1. Payment** | `lib/{order-finalize,slip-verify,slip-policy,easyslip,slipok,promptpay,slip-match,slip-date,slip-freshness,slip-image,payer-key,order-sweeper,ticket-limit,entry-code,holder-policy,booking-guards}.ts` (rev 40: `slip-verify` = สวิตช์ `SLIP_PROVIDER` · `slip-policy` = ด่านกลาง ต้องแนบสลิป/fail-closed/mock/บัญชีปลายทาง/transRef · `easyslip`/`slipok` = คุยกับผู้ให้บริการอย่างเดียว) · `app/actions/{booking,tickets}.ts` · `app/(public)/checkout/[orderId]/` · `app/(public)/account/tickets/` · `app/(admin)/admin/{refunds,checkin}/` · Prisma `Order/OrderItem/Payment/Ticket/TicketReturn` · tests `tests/unit/{easyslip,easyslip-errors,slipok,slip-verify,slip-*,payer-key,ticket-limit,entry-code,holder-policy,order-sweeper}.test.ts` · race `scripts/test-n1-race.ts`, `scripts/test-f1-f3.ts` · manual real-money test `scripts/test-concert.mjs` (สร้างคอนเสิร์ต 1 บาทจริง ระวังก่อนรัน) |
...
| **5. Admin & Cron** | `lib/{admin-guard,admin-stats}.ts` · `app/(admin)/*` · `app/actions/concert.ts` (สร้าง/แก้ไข/ลบคอนเสิร์ต — กติกาฟอร์มใน `lib/concert-form.ts` pure (rev 41: ปิดขาย ≤ เวลาแสดง, slug a-z0-9-, ลบได้เมื่อไม่มี order) · หน้าแก้ไข `app/(admin)/admin/concerts/[id]/edit/` + `components/concert-edit-form.tsx` (useActionState) · slug ผ่าน `lib/slug.ts`: ไทยล้วน → `concert-<id>`, ซ้ำ → `<slug>-<id>`; rev 39 แก้บั๊ก slug ว่างกดเข้าไม่ได้ · `toThaiDateTimeLocal` ใน `lib/local-datetime.ts`) · `app/api/cron/sweep/` (Vercel Cron รายวัน — `vercel.json` `0 0 * * *` ลิมิต Hobby) · `scripts/sweep-orders.ts` |
...
| **(cross-cutting) สถานะที่แสดงของคอนเสิร์ต** | `lib/concert-display.ts` (pure) — `deriveDisplayStatus()` = status ใน DB เทียบความจริง (ไม่มีโซน → NOT_READY "ยังไม่พร้อมขาย" · เลย `saleEndAt` → ENDED · ก่อน `saleStartAt` → SCHEDULED · **`eventAt` ผ่านแล้ว → ENDED ทุกสถานะ** rev 39) + `minZonePrice()` (null เมื่อไม่มีโซน — เดิม `Math.min([])` = Infinity โชว์ ฿∞) + `publicStatusHint()` (หน้าแอดมิน list/detail บอก "ผู้ชมเห็นเป็น …" เมื่อไม่ตรงกับป้ายที่ตั้ง) · ใช้ใน `components/concert-card.tsx`, `app/(public)/concerts/[slug]/page.tsx` (ป้าย+CTA+แถบ `?cancelled=1`), `app/(public)/concerts/[slug]/queue/page.tsx` (เปิดห้องรอเฉพาะ ON_SALE จริง), `app/page.tsx` (แถบตัววิ่ง), `app/(public)/concerts/page.tsx` (ตัวนับ), `app/(admin)/admin/concerts/{page,[id]/page}.tsx` — rev 37 (user-test #40) + rev 39 · **อย่าเช็ค `concert.status === "ON_SALE"` ตรง ๆ ในหน้า public อีก** ใช้ helper นี้ |
| **(cross-cutting) เอกสารผู้ใช้/PDPA + หน้า error + แชร์ลิงก์** | `lib/{legal-info,consent}.ts` · `components/legal-page.tsx` · `app/(public)/{privacy,terms,ticket-terms}/` (ตัวเลขดึงจาก env: SEAT_HOLD_TTL/RETURN_CUTOFF/HOLDER_MIN_AGE; `REFUND_DAYS`=14 ทีมยืนยันแล้ว; ช่องทางติดต่อ = `SUPPORT_EMAIL` → fallback `SEED_ADMIN_EMAIL` (rev 38); log กันบอท**ไม่ลบอัตโนมัติ**ตามที่ทีมตัดสิน; `LICENSE` = สงวนลิขสิทธิ์ (`package.json` license UNLICENSED)) · checkbox ยินยอมใน `components/register-form.tsx` + **server ตรวจซ้ำ** ใน `app/actions/auth.ts` (`hasAcceptedTerms`) + หมายเหตุใต้ปุ่ม Google (`google-signin-button.tsx`) · แจ้งเก็บ fingerprint ที่หน้า queue · บรรทัดเงื่อนไขบัตรบน checkout · ลิงก์ในฟุตเตอร์ · `app/{not-found,error,global-error}.tsx` + `loading.tsx` (concerts, [slug], checkout) · `app/{icon,apple-icon,opengraph-image,brand-mark}.tsx` วาดด้วย `next/og` (Satori: ห้ามใส่สีทึบใน `background` shorthand → แยก `backgroundColor`; ข้อความในรูปเป็นอังกฤษเพราะฟอนต์ตั้งต้นไม่มีไทย) + `metadataBase/openGraph` ใน `app/layout.tsx` + `generateMetadata` ต่อคอนเสิร์ต (โปสเตอร์เป็น og:image) · `app/{robots,sitemap}.ts` — rev 34 (2026-08-27). ⚠️ dev server: เขียนไฟล์ใหม่หลายไฟล์พร้อมกันแล้ว import ชี้ไฟล์ที่ยังไม่มี = static worker ของ `next dev` ตาย 2 ครั้งแล้ว**ค้างถาวร** (`/concerts/[slug]` 500 "Jest worker … exceeding retry limit") — แก้ด้วยรีสตาร์ท dev server เท่านั้น |
...
- **Unit**: `tests/unit/*.test.ts` — **57 ไฟล์, 705 cases** (รันจริง 2026-08-27 หลัง rev 41 — เพิ่ม `concert-form` 12; rev 40 เพิ่ม `slipok` 15, `slip-verify` 6; rev 39 เพิ่ม `slug` 7, `easyslip-errors` 9, `concert-display` +5; ก่อนหน้า 52 ไฟล์ 647 หลังรวม rev 31–37 — เพิ่ม `queue-soldout-gate`, `turnstile`, `money`, `local-datetime`, `email-signup-gate`, `seed-policy`, `credentials-auth-verify-flag`, `canonical-host`, `consent`, `password-reset`, `email-templates`, `health`, `concert-display`; rev 28 +4, rev 29 +6, rev 30 +4, rev 31 +11 seed-policy, rev 32 +6 email-signup-gate/+4 credentials-auth-verify-flag, rev 33 +7 canonical-host, rev 34 +6 consent, rev 35 +11 password-reset/+8 email-templates, rev 36 health, rev 37 +8 concert-display) · **CI รัน lint + `next build` ด้วยแล้ว (rev 37)** — job integration มี Postgres+Redis service, Vitest, mock ล้วนไม่ต้องมี DB/Redis จริง (`pnpm test:run` — **ห้าม `pnpm test` = watch ค้าง**)
...
`prisma/migrations/` (16 folder เล็กๆ — 8 master + 5 seatmap + 2 presale + 1 data-fix rev 39, ลง Neon ครบแล้ว ห้ามลบ) **ไม่ต้อง exclude** — อ่านได้เต็มถ้าต้องการ ไม่ใหญ่.
