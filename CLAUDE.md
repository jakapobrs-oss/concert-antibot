...
- **Unit**: `tests/unit/*.test.ts` — 42 ไฟล์, 570 cases (รันจริง 2026-08-26 — เพิ่ม `queue-soldout-gate`, `turnstile`, `money`), Vitest, mock ล้วนไม่ต้องมี DB/Redis จริง (`pnpm test:run` — **ห้าม `pnpm test` = watch ค้าง**)
