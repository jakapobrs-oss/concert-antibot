...
| `12_CHANGELOG.md` | ประวัติ session — ล่าสุดคือ Revision 28 (2026-08-26 บ่าย: prod ใช้ Turnstile test key มา 43 วัน → guard fail-closed + ห้องรอมี feedback); 27 = SECURITY_TODO #2–#4; 26 = คิวมีทางออกเมื่อบัตรหมด; 25 = merge สาย presale (2026-08-25). เลข 18–22 มีสองชุด (สาย seatmap ด้านบน / สาย presale ใต้ป้ายแยก) เพราะเขียนคู่ขนาน. **มีช่องว่าง rev 17→18** ไม่รวมงาน named-ticket + 7-part Codex review (commit ถึง 2026-07-10) |
...
- **Unit**: `tests/unit/*.test.ts` — 42 ไฟล์, 574 cases (รันจริง 2026-08-26 บ่าย — เพิ่ม `queue-soldout-gate`, `turnstile`, `money`; rev 28 +4), Vitest, mock ล้วนไม่ต้องมี DB/Redis จริง (`pnpm test:run` — **ห้าม `pnpm test` = watch ค้าง**)
