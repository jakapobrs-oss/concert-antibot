...
| **7. Infra** | `lib/{env,env-schema,prisma,redis,json,format,local-datetime,get-ip,email}.ts` (`local-datetime` = ตีความ datetime-local จากฟอร์มแอดมินเป็นเวลาไทย — rev 29) · `next.config.ts` · `docker-compose.yml` · `.github/workflows/ci.yml` |
...
- **Unit**: `tests/unit/*.test.ts` — 43 ไฟล์, 580 cases (รันจริง 2026-08-26 เย็น — เพิ่ม `queue-soldout-gate`, `turnstile`, `money`, `local-datetime`; rev 28 +4, rev 29 +6), Vitest, mock ล้วนไม่ต้องมี DB/Redis จริง (`pnpm test:run` — **ห้าม `pnpm test` = watch ค้าง**)
