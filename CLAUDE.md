...
| **7. Infra** | `lib/{env,env-schema,prisma,redis,json,format,local-datetime,get-ip,email,email-signup-gate}.ts` (`local-datetime` = ตีความ datetime-local จากฟอร์มแอดมินเป็นเวลาไทย — rev 29 · `email-signup-gate` = production ไม่มี Resend → ปิดรับสมัครด้วยอีเมล fail-closed + ส่งเมลไม่ออกถอนบัญชี — rev 30) · `next.config.ts` · `docker-compose.yml` · `.github/workflows/ci.yml` · `scripts/push-env-to-vercel.mjs` (ก๊อป env จาก `.env` ขึ้น Vercel โดยไม่พิมพ์ค่า) |
...
- **Unit**: `tests/unit/*.test.ts` — 44 ไฟล์, 584 cases (รันจริง 2026-08-26 ค่ำ — เพิ่ม `queue-soldout-gate`, `turnstile`, `money`, `local-datetime`, `email-signup-gate`; rev 28 +4, rev 29 +6, rev 30 +4), Vitest, mock ล้วนไม่ต้องมี DB/Redis จริง (`pnpm test:run` — **ห้าม `pnpm test` = watch ค้าง**)
