...
// ฟอร์มขอลิงก์ยืนยันอีเมลใหม่ — ข้อความสำเร็จเป็นกลาง (ไม่บอกว่ามีบัญชี/ยืนยันแล้วหรือยัง)
import { useActionState } from "react";
import { AlertCircle, MailCheck } from "lucide-react";
import { resendVerificationAction } from "@/app/actions/password";
import { VERIFICATION_RESENT_MESSAGE } from "@/lib/password-reset";
  ...
  if (state?.ok) {
    return (
      <div role="status" className="flex items-start gap-2.5 rounded-lg border border-success/25 bg-success/10 p-3 text-sm text-success">
        ...
        <span>{VERIFICATION_RESENT_MESSAGE}</span>
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <Button type="submit" size="lg" className="w-full" loading={isPending}>
        {isPending ? "กำลังส่ง…" : "ส่งลิงก์ยืนยันใหม่"}
      </Button>
    </form>
  );
}
